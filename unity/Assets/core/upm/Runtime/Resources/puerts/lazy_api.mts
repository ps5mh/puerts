...
const REGISTER_LAZY_API = function () {
    ...
    function addExtensionAPI(jsClass: JSClass, apiName: string, extClses: JSClass[] | undefined) {
        ...
        const ok = wrapAPI(jsClass, apiName, api, false, MemberTypes.Method, flags);
        if (ok) {
    ...
    function addEnumAPI(jsClass: JSClass, apiName: string) {
        if (!isNaN(+apiName)) {
        ...
        const ok = addAPI(jsClass, apiName, true, MemberTypes.Field);
        if (ok) {
            return true;
        }
        return false;
    ...
    function addNestedType(jsClass: JSClass, apiName: string, innerType?: any) {
        const csType = puerts.$typeof(jsClass);
    ...
    function addAPI(
        ...
        const csType = puerts.$typeof(jsClass);
        if (!csType) {
            LL.W >= config.LL && log(LL.W, 'bad state, puerts.$typeof(jsClass) is null', jsClass, apiName, isStatic);
            return false;
        }
        const bindingFlags =
    ...
    function addAPIHierarchy(
        ...
        try {
            ...
            LL.E >= config.LL && log(LL.E, `register api failed! With exception: ${e}`, jsClass, apiName.toString(), isStatic);
        }
        return false;
    }
    ...
    function setupSystemObjectInstanceLazyProxy() {
        ...
        instanceProxyHandler.get = function (t, p, r) {
            ...
            const jsClass: JSClass = r.constructor;
        ...
        instanceProxyHandler.set = function (t, p, v, r) {
            ...
            const jsClass: JSClass = r.constructor;
            return addAPIHierarchy(jsClass, p, false, MemberTypes.Field | MemberTypes.Property)
                ? ((receiver[p] = v), true)
                : Reflect.set(t, p, v, r);
        };
    ...
    function setupSystemObjectStaticLazyProxy() {
        ...
        staticProxyHandler.set = function (t, p, v, r) {
            ...
            const receiver = r;
            return addAPIHierarchy(r, p, true, MemberTypes.Field | MemberTypes.Property)
                ? ((receiver[p] = v), true)
                : Reflect.set(t, p, v, r);
        };
